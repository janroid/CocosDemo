...
local function getAuthor()
	...
	local authorMap = {
		...
		["TaylorTan"] = "TaylorTan";
		["JanRoid"] = "JanRoid";
		["JohnsonZhang"] = "JohnsonZhang";
		["AllenLuo"] = "AllenLuo";
		["ReneYang"] = "ReneYang";
		["LoyalwindPeng"] = "LoyalwindPeng";
	};
...
M.create = function()
	...
	-- UI模板
	local file = modDir .. M.modName .."Pop.lua";
	if io.exists("TemplatePop.lua") and (not io.exists(file)) then
		local content = io.readfile("TemplatePop.lua");
		content = string.replaceAll(content,"Template",M.modName)
		content = string.format(content,Author,time);
		io.writefile(file, content);
	end
	...
	if io.exists("init.lua") and (not io.exists(file)) then
		...
		content = string.replaceAll(content,"Template",M.modName)
		content = string.replaceAll(content,"scene = require",M.modName.."Pop = require")
		content = string.format(content,Author,time);
		io.writefile(file, content);
	end
