...
M.create = function()
	...
	-- UI模板
	local file = modDir .. M.modName .."View.lua";
	if io.exists("TemplateView.lua") and (not io.exists(file)) then
		local content = io.readfile("TemplateView.lua");
		content = string.replaceAll(content,"Template",M.modName)
		content = string.format(content,Author,time);
		io.writefile(file, content);
	end
