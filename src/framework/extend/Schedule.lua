...
--[[
	调度 fn 每隔 interval``(秒)执行一次，共执行 ``count 次。
	delay 默认为不传入， 表示下一次循环立即开始执行。
	count 默认为 -1， 表示执行无数次。
	返回值是 Event 对象。
]]
function Schedule:schedule(fn,interval,delay,count)
	local eventObj;
	interval = numberLib.valueOf(interval);
	local runTime = delay and interval or 0;
	delay = numberLib.valueOf(delay);
	count = numberLib.valueOf(count,-1);
	local runCount = 0;
	local sleep_time = 0;
	...
	local extend = {
		...
		sleep = function(self,interval)
			print("定时器休眠",interval)
