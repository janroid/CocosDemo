...
-- 接收socket消息，通知对象处理
function NetManager:parseMsg(nNetSocketId,msgType,msgData,msgSize)
	if self.m_netServers[nNetSocketId] then
		self.m_netServers[nNetSocketId]:parseMsg(msgType,msgData,msgSize)
	end
end
...
function NetManager:sendMsgToCurServer(nNetSocketId,msgType,msgData)
	if self.m_netServers[nNetSocketId] then
		NativeCall.lcc_sendMsgToServer(nNetSocketId,msgType,msgData)
	end
end
...
-- 是否socket对象
function NetManager:removeServer(nNetSocketId)
