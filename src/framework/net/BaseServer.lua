...
function BaseServer:onNetEventHandler(nEventId, pEventArg)
	...
	if g_SocketCmd.SERVER_STATUS_CONNECTING == nEventId then -- 0
		-- 开始连接socket
		self.m_nServerStatus = g_SocketCmd.g_SocketCmd.SOCKET_STATUS_CONNECTING;
		self:onConnectBegin(g_SocketCmd.SOCKET_EVENT_CONNECT_BEGIN,pEventArg);
	elseif g_SocketCmd.SOCKET_STATUS_CONNECTFAIL == nEventId then -- 2
		-- socket连接失败
		self.m_nServerStatus = g_SocketCmd.SOCKET_STATUS_CONNECTFAIL
		self:onConnectFailed(g_SocketCmd.SOCKET_EVENT_CONNECT_FAILED,pEventArg);


	elseif g_SocketCmd.SOCKET_STATUS_DISCONNECT == nEventId then -- 3
		-- 关闭socket
		self.m_nServerStatus = g_SocketCmd.SOCKET_STATUS_DISCONNECT
		-- self:onDisconnect(g_Event.SOCKET_EVENT_CLOSED,pEventArg);
	elseif g_SocketCmd.SOCKET_STATUS_CONNECTED == nEventId then -- 1
		-- socket连接成功
		self.m_nServerStatus = g_SocketCmd.SOCKET_STATUS_CONNECTED
		self:onConnectComplete(g_SocketCmd.SOCKET_EVENT_CONNECT_COMPLETE,pEventArg);
    end
end
...
-- 发送socket消息
function BaseServer:sendMsg(data)
	NativeCall.lcc_sendMsgToServer(self.m_nNetSocketId,data)
end
...
function BaseServer:parseMsg(serverType,msgType,msgData,msgSize)
	g_EventDispatcher:dispatch(serverType,msgType,msgData,msgSize);
