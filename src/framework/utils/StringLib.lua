...
function string.urlencode(str)
	...
	---escape all characters but alphanumeric, '.' and '-', '=', '_', ':', '/','?', '&', '%', ' '
...
function string.urldecode(str)
	...
	while true do
		local temp = string.gsub(str, '%%(%x%x)', function(hex)
...
-- 邮箱格式验证
function string.isRightEmail(str)
	...
	--首字母不能是 . _ -
	local firstStr = string.sub(bstr, 1, 1)
	if firstStr=="." or firstStr=="_" or firstStr=="-" then
		return false
	end
	bstr = string.gsub(bstr, "%.", "") --可以有 . 
	bstr = string.gsub(bstr, "_", "")  --可以有 _ 
	bstr = string.gsub(bstr, "-", "") --可以有 -
...
--判断一个字符串是否是MAC
function string.isStringMac(str)
    ...
    for i=1,nLen do
        ...
        if(i == 3 or i == 6 or i == 9 or i == 12 or i == 15) then
            ...
            if(ch ~= "0" and ch ~= "1" and ch ~= "2" and ch ~= "3" and ch ~= "4" and ch ~= "5" and ch ~= "6" and ch ~= "7" and ch ~= "8" and ch ~= "9"
                and ch ~= "A" and ch ~= "B" and ch ~= "C" and ch ~= "D" and ch ~= "E" and ch ~= "F") then
                return false;
            end
        end
    end
...
-- param fontSize 文字大小
-- param length 長度限制
function string.limitLength(str,fontSize,length)
	if not length or length <= 0 then
		return str
	end
	local tempStr = cc.Label:createWithSystemFont(str,  "fonts/arial.ttf", fontSize)
	local tempSize = tempStr:getContentSize()
	if tempSize.width <= length then
		tempStr = nil
		return str
	else
		local isBreak = false
		...
		repeat
			i = i-1
			local temp = string.utf8SubStr(str,i) .. "..."
			tempStr:setString(temp)
			local subStrSize = tempStr:getContentSize()
			if subStrSize.width <= length then
				isBreak = true
				str  = temp
			end
		until isBreak
...
string.substitute = function(str,...)
	local ret = str;
	local arg = {...}
    if str ~= nil then
       for i = 1, #arg do
...
--[Comment]
--数组拼接
string.join2 = function(t, sep)
    sep = sep or "."
    local str = "";
    if t ~= nil then
        for i = 1, #t do
            if type(t[i]) == "table" then
                str = str .. sep .. "{" .. string.join(t[i], sep) .."}";
            else
                str = str .. sep..tostring(t[i]);
            end
        end
    end
    if sep ~= "" and string.indexOf(str, sep) == 1 then
