...
--[[--
将来源表格中所有键及其值复制到目标表格对象中，如果存在同名键，则覆盖目标表的值
@usage
...
function TableLib.indexof(arr, item, index)
    if arr ~= nil then
        index = index or 1;
        if index < 0 then
            index = #arr + 1 + index;
...
--[[--
    复制table，只复制table，对table内的其他无效
]]
function TableLib.copyTab1(st)
    local tab = {}
    for k, v in pairs(st or {}) do
        if type(v) == "table" then
            tab[k] = TableLib.copyTab(v)
...
-- 快速排序,排序整个table
-- @param list 待排序列表
-- @param obj,func 排序规则函数
TableLib.quickSort = function(list, obj, func)
    return TableLib.quick_sort(list, 1, #list, obj, func);
end

--[Comment]
--快速排序,排序left - right
TableLib.quick_sort = function (list, left, right, obj, func)
    if left < right then
        local i = left;
        local j = right;
        local pivot = list[left];
        while i < j do
        --func(list[right], pivot)) == -1 -> 小于
        --func(list[right], pivot)) == 0  -> 等于
        --func(list[right], pivot)) == 1  -> 大于
        ...
        list[i] = pivot;  
        TableLib.quick_sort(list, left, i - 1, obj, func); 
        TableLib.quick_sort(list, i + 1, right, obj, func);  
    end
    return list;
end
