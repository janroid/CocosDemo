local RoomTaskItem  = class("RoomTaskItem",cc.TableViewCell)
...
RoomTaskItem.DEFAULT_TASK_ICON= "creator/dailyTask/imgs/icon_unload.png"

BehaviorExtend(RoomTaskItem)

function RoomTaskItem:ctor(data)   
    self:bindBehavior(BehaviorMap.DownloadBehavior);
    self:init(data)
end 

function RoomTaskItem:dtor()

end

function RoomTaskItem:init(data)   
    local root = g_NodeUtils:getRootNodeInCreator('creator/dailyTask/roomTaskItem.ccreator')
    self.m_root = g_NodeUtils:seekNodeByName(root,"root");
    self.m_root:setAnchorPoint(0,0)
    self:addChild(self.m_root)    

    self.m_imgItemBg            = g_NodeUtils:seekNodeByName(self.m_root,'room_task_item_bg');
    ...
    self.m_txTaskDec            = g_NodeUtils:seekNodeByName(self.m_root,'label_task_desc');
    self.m_txReward             = g_NodeUtils:seekNodeByName(self.m_root,'label_task_reward');
    self.m_nodeProgress         = g_NodeUtils:seekNodeByName(self.m_root,'progress_bar');
    self.m_progressBg           = g_NodeUtils:seekNodeByName(self.m_root,'img_progress_bg');
    ...
    self.m_btnGetReward         = g_NodeUtils:seekNodeByName(self.m_root,'btn_reward');
    self.m_txGetReward          = g_NodeUtils:seekNodeByName(self.m_root,'label_reward');
    self.m_txtUnderWay          = g_NodeUtils:seekNodeByName(self.m_root,'label_underway');
    ...
    self.m_txGetReward:setString(GameString.get("str_achi_get_reward")..GameString.get("str_new_mtt_reward"))
...
function RoomTaskItem:getSpecialColor(special,color1,color2)
    local r, g, b = 0, 0, 0;
    local a = 255
    if special then
...
function RoomTaskItem:getSize()
    return self.m_root:getContentSize()
end

function RoomTaskItem:updateCell(data)
    -- {
    --     "m_isLevelTask": false,
    --     "m_target": 6,
    --     "m_status": 0,
    ...
    local s = self.m_txReward:getContentSize()
    if s.height > 50 then
        self.m_txReward:setScale(0.8)
    end
    ...
    if self.m_data.m_status == TaskVO.STATUS_UNDER_WAY then
...
function RoomTaskItem:addIcon(data,view)
    ...
    self:downloadImg(url,function(self,data)
        if data.isSuccess then 
            ...
            view:setTexture(RoomTaskItem.DEFAULT_TASK_ICON)
        end
        view:setContentSize(size) 
     end)
end

function RoomTaskItem:getIconUrl(data)
...
function RoomTaskItem:onBtnGetRewardClick()
...
function RoomTaskItem:onGetRewardCallback(isSuccess,data)
    if g_TableLib.isTable(data) then
        -- 领奖成功
        Log.d("DailyTask get reward success! data = ",data) --動畫資源暫無
        --[[
            ret == 0 --其他情况
            ret == 1 --领取成功
