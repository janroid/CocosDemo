...
local MttResultCtr = class("MttResultCtr",PopupCtr);
BehaviorExtend(MttResultCtr);

---配置事件监听函数
MttResultCtr.eventFuncMap =  {
	-- ["EventKey"] = "FuncName"
	-- EventKey必须定义在SceneEvent.lua中
	-- 与UI的通信调用参见PopupCtr.updateView函数	
}

function MttResultCtr:ctor()
	PopupCtr.ctor(self);
end

function MttResultCtr:show()
	PopupCtr.show(self)
end

function MttResultCtr:hidden()
	PopupCtr.hidden(self)
end

function MttResultCtr:onCleanup()
...
function MttResultCtr:onEnter()
	PopupCtr.onEnter(self, false);
	-- xxxxxx
end

function MttResultCtr:onExit()
