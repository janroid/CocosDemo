...
local UpdateCtr = class("UpdateCtr",ViewCtr);
BehaviorExtend(UpdateCtr);

---配置事件监听函数
UpdateCtr.s_eventFuncMap =  {
	-- ["EventKey"] = "FuncName"
	-- EventKey必须定义在SceneEvent.lua中
	-- 与UI的通信调用参见ViewCtr.updateView函数
}

function UpdateCtr:ctor()
	ViewCtr.ctor(self);

	
end

function UpdateCtr:onEnter()
	-- do something
	--[[
...
function UpdateCtr:onEnterTransitionDidFinish()
	-- do something
	--[[
		进入场景过渡动画播放结束时被调用
		如果场景没有过渡动画，该方法不会被调用
...
function UpdateCtr:onExitTransitionDidStart()
	-- do something
	--[[
		退出场景动画播放前会被调用
		如果场景没有过渡动画，该方法不会被调用
