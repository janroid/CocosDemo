...
public class APNUtil {
    ...
    public static final String ANP_NAME_2G = "2G";
    public static final String ANP_NAME_3G = "3G";
    public static final String ANP_NAME_4G = "4G";
    ...
    public static String getApn(Context context) {
        ...
        String res = null;
        try {
            c = context.getContentResolver().query(PREFERRED_APN_URI, new String[]{APN_PROP_APN},
                    null, null, null);
            if (c != null) {
                c.moveToFirst();
                if (!c.isAfterLast()) {
                    res = c.getString(c.getColumnIndex(APN_PROP_APN));
                }
            }
        } catch (Exception e) {
        } finally {
    ...
    public static String getApnProxy(Context context) {
        ...
        try
        {
            c = context.getContentResolver().query(PREFERRED_APN_URI, new String[]{APN_PROP_PROXY},
                    null, null, null);
            //很多变态中兴手机报空指针错误
            if (c != null)
            {
                c.moveToFirst();
                if (!c.isAfterLast()) {
                    res = c.getString(c.getColumnIndex(APN_PROP_PROXY));
                }
            }
        }
        catch (Exception e) {
    ...
    public static String getApnPort(Context context) {
        ...
        try
        {
            c = context.getContentResolver().query(PREFERRED_APN_URI, new String[]{APN_PROP_PORT},
                    null, null, null);
            if (c != null)
            {
                c.moveToFirst();
                if (!c.isAfterLast()) {
                    res =  c.getString(c.getColumnIndex(APN_PROP_PORT));
                }
            }
        }
        catch (Exception e) {
        ...
        if (null != info) {
            ...
            if (info == null) {
                ...
                if (subType == 4 || subType == 1
                        || subType == 2) {
                    name = ANP_NAME_2G;
                } else if (subType == 3 || subType == 8
                        || subType == 6 || subType == 5
                        || subType == 12) {
                    name = ANP_NAME_3G;
                } else if (subType == 13) {// LTE是3g到4g的过渡，是3.9G的全球标准
                    name = ANP_NAME_4G;
                }
            }
        }
    ...
    /**
     * 返回运营商类型  移动 1       联通2      电信3   无0
     *
     * @return
     */
//    public static int getMobileType() {
//        if (SimUtil.isChinaMobile()) {
//            return 1;
//        } else if (SimUtil.isChinaUnicom()) {
//            return 2;
//        } else if (SimUtil.isChinaTelecom()) {
//            return 3;
//        }
//        return 0;
//    }
...
//        int type = getMobileType();
//        switch (type) {
//            case 1:
//                return GameString.getString("yd");
//            case 2:
//                return GameString.getString("lt");
//            case 3:
//                return GameString.getString("dx");
//        }
//        return "unkown";
//    }
}