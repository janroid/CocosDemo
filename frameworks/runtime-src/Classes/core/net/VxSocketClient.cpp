...
VxSocketClient::VxSocketClient(const char* addr, unsigned short port, bool bUseTLS)
{
	m_addr = addr;
	m_port = port;
	...
	m_bDeleteAvailable = true;
	m_bRecvThreadAvailable = false;
	m_bSendThreadAvailable = false;
	m_bSendMsgAvailable = false;
	m_bUseTLS = bUseTLS;
	m_bContinueToRecvAvailable = true;
	if (bUseTLS)
	{
		m_socket = new VxTLSSocket();
	}
	else
	{
		m_socket = new VxSocket();
	}
	
	CCLOG("VxSocketClient end");
}
...
VxSocketClient::~VxSocketClient()
{
	...
	VX_SAFE_DELETE(m_recvThread);
	VX_SAFE_DELETE(m_sendThread);
	delete m_socket;
...
void* VxSocketClient::_sendThreadProc(void* arg)
{
	...
	if(client->m_state == VXSOCKET_STATE_CONNECTING)
	{
		do 
		{
			...
			client->m_bDeleteAvailable = false;
			if(!client->m_socket->create()
				//|| !client->m_socket->parseDns(client->m_addr.c_str(), ip)
				|| !client->m_socket->connect(client->m_addr.c_str(), client->m_port))
			{
				
				client->_close();
				client->_destroyThreadResource();
				client->notifyEvent(VXSOCKET_EVENT_CONNECT_FAILED, NULL);
				client->m_bDeleteAvailable = true;
				return NULL;
			}
			client->m_bDeleteAvailable = true;
...
{
	//m_bDeleteAvailable = false;
	m_bRecvThreadAvailable = true;
	while(m_state == VXSOCKET_STATE_RUNNING)
	{
		...
		m_bContinueToRecvAvailable = false;
...
bool VxSocketClient::isDeleteAvailable()
{
	return m_bDeleteAvailable && m_bRecvThreadAvailable== false && m_bSendThreadAvailable== false;
}

bool VxSocketClient::enableTLS(bool bEnable)
{
	return m_socket->enableTLS(bEnable);
}

bool VxSocketClient::enableCompress(bool bEnable)
{
	return m_socket->enableCompress(bEnable);
}

bool VxSocketClient::continueToRecv()
{
	//VXLOG("VxSocketClient::continueToRecv, begin");
	m_recvSem.notify_one();
	//VXLOG("VxSocketClient::continueToRecv, end");
	m_bContinueToRecvAvailable = true;
	return  true;
}
