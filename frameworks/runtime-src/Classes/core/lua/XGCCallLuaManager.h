...
public:
	...
	void loop(float dt);
public:
	void clearSocketTempEventData();
	void addSocketTempEventData(int nNetSocketId, int eventId, void* argv);
	void addSocketEventData(int nNetSocketId, int eventId, void* argv);

	void socketEvent(int nNetSocketId, int eventId, void* argv);
	void recvMsg(int nNetSocketId ,int msgType, int msgSize, char* argv);
